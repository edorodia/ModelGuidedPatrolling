...
@numba.jit(nopython=True)
def compute_algae_bloom_map(T, M, N, nav_map, wind, dt=0.1, v_wind = 1, v_current = 1):
    ...
    num_of_particles_per_bloom = np.array([np.random.randint(1,300) + 80 for _ in range(number_of_starting_points)])
    ...
    for t in range(T):
        ...
        for idx in range(total_num_of_particles):
            ...
            particles[idx] = particles[idx] + (v_current*contour_current + current_field(particles[idx], M, N) + v_wind*wind) * dt
        ...
        algae_smooth_map[t] = gaussian_filter_2d(algae_map[t], 1) 
...
class AlgaeBloomGroundTruthNumba:
    ...
    def reset(self):
        ...
        self.wind = np.asarray([np.random.rand()*2 -0.5, np.random.rand()*2 -0.5])
        self.algae_bloom_map = compute_algae_bloom_map(self.max_steps, self.grid.shape[0], self.grid.shape[1], self.grid, self.wind, dt=self.dt, v_current=1, v_wind=0.5)
        self.t = self.t0
...
if __name__ == "__main__":
    ...
    M = 58
    N = 60
    nav_map = np.genfromtxt("groundTruths\Maps\map.txt")
