...
class CoordinatedFleet:
	...
	def get_vehicle_trajectory_map(self, observer: int):
		...
		position_map[waypoints[-10:, 0].astype(int), waypoints[-10:, 1].astype(int)] = np.linspace(0, 1, len(waypoints[-10:]), endpoint=True)
...
class DiscreteModelBasedPatrolling:
	...
	def __init__(self,
			...
			min_information_importance: float = 0.05,
	):
		...
		for vehicle_id in self.fleet.vehicles_ids:
			...
			if self.int_observation:
				...
				observations[vehicle_id] = np.concatenate((
					...
					(255 * self.fleet.get_vehicle_trajectory_map(observer=vehicle_id)[np.newaxis]).astype(np.uint8),
	...
	def get_done(self):
		...
		done = {agent_id: self.fleet.vehicles[agent_id].distance > self.max_distance for agent_id in
