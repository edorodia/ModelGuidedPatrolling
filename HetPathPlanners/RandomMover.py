...
class RandomDroneMover:

	def __init__(self, world: np.ndarray):
		
		self.world = world
		self.possible_positions = np.argwhere(self.world == 1)
	"""
	returns the action for the drone, random action that is not the same as the actual drone one
	"""
	def move(self, actual_position):
		filtered_positions = np.array([row for row in self.possible_positions if not np.array_equal(row, actual_position)])
		return filtered_positions[np.random.choice(filtered_positions.shape[0])]
		

class RandomVehicleMover:
	...
	"""
	returns the action for the vehicle
	"""
	def move(self, actual_position):
	...
	"""
	converts the action of the vehicle into the x,y direction to take
	"""
	...
	"""
	finds the opposite action to the one given
	"""
	...
	"""
	checks for a collision doing the action given from the position given
	"""
