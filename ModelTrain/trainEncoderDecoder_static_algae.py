...
from Models.VAE import EncoderDecoder
import torch as th
import numpy as np
...
dataloader_test = DataLoader(dataset_test, batch_size = 128, shuffle = True, num_workers = 0)
...
# Import the model
loss_weights = {'recon': 1, 'features': 0.1, 'kl': 1}
model = EncoderDecoder(input_size=dataset[0][0].shape, latent_size=256, output_channels=1, loss_weights = loss_weights).to(device)
...
dir_path = 'runs/TrainingEncoderDecoder/EncDec_Algae_{}'.format(time.strftime("%Y%m%d-%H%M%S"))
os.system('rm -rf ' + dir_path)
writer = tb.SummaryWriter(log_dir=dir_path, comment='EncDec_static_training_algae')
...
for epoch in track(range(N_epochs), description="Training progress: "):
	...
	th.save(model.state_dict(), dir_path + '/EncDec_static_algae_train.pth'.format(epoch))

	# Save the model if the loss is lower than the previous one
	if epoch == 0:
		th.save(model.state_dict(), dir_path + '/EncDec_static_algae.pth')
		min_loss = np.mean(running_test_loss)
	elif np.mean(running_test_loss) < min_loss:
		th.save(model.state_dict(), dir_path + '/EncDec_static_algae_test.pth'.format(epoch))
