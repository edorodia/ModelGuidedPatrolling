...
dir_path = 'runs/TrainingVAE/VAE_Algae_{}'.format(time.strftime("%Y%m%d-%H%M%S"))
os.system('rm -rf ' + dir_path)
writer = tb.SummaryWriter(log_dir=dir_path, comment='VAE_static_training_algae')
...
for epoch in track(range(N_epochs), description="Training progress: "):
	...
	if epoch == 0:
		th.save(model.state_dict(), dir_path + '/VAE_static_algae.pth')
		min_loss = np.mean(running_test_loss)
	elif np.mean(running_test_loss) < min_loss:
		th.save(model.state_dict(), dir_path + '/VAE_static_algae_{}.pth'.format(epoch))
