...
class A2Cagent:
	...
	def run_evaluation(self, path: str, algorithm: str, reward_type: str, runs: int, n_agents: int, ground_truth_type: str, render = False):
		...
		for run in trange(runs):
			...
			U0 = self.env.gp_coordinator.sigma_map.sum()
			metrics['Total uncertainty'].append(self.env.gp_coordinator.sigma_map.sum() / U0)
			metrics['$\Delta \mu$'].append(0)
			metrics['$\Delta \sigma$'].append(0)
			metrics['Error $\mu$'].append(self.env.get_error())
			metrics['Max. Error in $\mu_{max}$'].append(1)
			metrics['Mean Error in $\mu_{max}$'].append(1)
			peaks, vals = find_peaks(self.env.gt.read())
			positions = self.env.fleet.get_positions()
			for i in range(n_agents): 
			...
			while not all(done.values()):
				...
				metrics['N_agents'].append(n_agents)
				metrics['Ground Truth'].append(ground_truth_type)
				metrics['Mean distance'].append(self.env.fleet.get_distances().mean())

				# Datos de cambios en la incertidumbre y el mu
				changes_mu, changes_sigma = self.env.gp_coordinator.get_changes()
				...
				# Error en el mu max
				peaks, vals = find_peaks(self.env.gt.read())
				estimated_vals = self.env.gp_coordinator.mu_map[peaks[:,0], peaks[:,1]]
