...
from PathPlanners.DRL.Networks.network import DuelingVisualNetwork, NoisyDuelingVisualNetwork, DistributionalVisualNetwork
...
class MultiAgentDuelingDQNAgent:
	...
	def __init__(
		...
		# The PER will store the transitions in order #
		self.memory = PrioritizedReplayBuffer((self.env.max_num_steps, *obs_dim), memory_size, batch_size, alpha=alpha)

		""" Create the DQN and the DQN-Target (noisy if selected) """
		self.dqn = RecurrentDuelingVisualNetwork(obs_dim, action_dim, number_of_features).to(self.device)
		self.dqn_target = RecurrentDuelingVisualNetwork(obs_dim, action_dim, number_of_features).to(self.device)

		# Save the hidden states of the recurrent network to avoid re-computing them #
		self.hidden_state = torch.zeros((self.env.number_of_agents, 1, 1, 512)).to(self.device)
	...
	def select_masked_action(self, states: dict, positions: dict, deterministic: bool = False):
		...
		for agent_id, state in states.items():
			...
			if self.epsilon > np.random.rand() and not self.noisy and not deterministic:
				...
				hidden_state = self.hidden_state[agent_id].clone()
				q_values = self.dqn.forward_with_hidden(torch.FloatTensor(state).unsqueeze(0).to(self.device), ).detach().cpu().numpy()
